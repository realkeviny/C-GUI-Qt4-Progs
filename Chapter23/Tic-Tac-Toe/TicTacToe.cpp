...
TicTacToe::TicTacToe(QWidget* parent /* = nullptr */) :QWidget(parent)
{
	clearBoard();
...
bool TicTacToe::threeInARow(int row1, int col1, int row3, int col3) const
{
	int row2 = (row1 + row3) / 2;
	int column2 = (col1 + col3) / 2;

	return board[row1][col1] != Empty && board[row2][column2] == board[row1][col1] && board[row3][col3] == board[row1][col1];
}