...
AddressBook::~AddressBook()
{
    ...
    if(file.open(QIODevice::WriteOnly))
    {
        ...
        {
            ABItem* entry = static_cast<ABItem*>(treeWidget->topLevelItem(i));
...
void AddressBook::createActions()
{
    ...
    addEntryAction->setStatusTip(tr("Add a new entry to the address book"));
    ...
    editEntryAction->setStatusTip(tr("Edit the current address book entry"));
    connect(editEntryAction,SIGNAL(triggered()),this,SLOT(editEntry()));

    deleteEntryAction = new QAction(tr("&Delete Entry"),this);
    deleteEntryAction->setShortcut(QKeySequence::Delete);
    deleteEntryAction->setStatusTip(tr("Delete the current address book entry"));
    connect(deleteEntryAction,SIGNAL(triggered()),this,SLOT(deleteEntry()));
}
...
{
    QList<QTreeWidgetItem*> items = treeWidget->findItems(contact, Qt::MatchExactly,0);
    if(items.isEmpty())
    {
        return 0;
    }
