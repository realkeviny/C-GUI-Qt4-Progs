...
Teapots::Teapots(QWidget* parent /* = nullptr */) :QGLWidget(parent)
{
	rubberBandIsShown = false;

	makeCurrent();
	fbObject = new QGLFramebufferObject(1024, 1024, QGLFramebufferObject::Depth);
...
{
	static constexpr GLfloat ambient[] = { 0.0,0.0,0.0,1.0 };
	static constexpr GLfloat diffuse[] = { 1.0,1.0,1.0,1.0 };
	static constexpr GLfloat position[] = { 0.0,3.0,3.0,0.0 };
	static constexpr GLfloat lmodelAmbient[] = { 0.2,0.2,0.2,1.0 };
	static constexpr GLfloat localView[] = { 0.0 };
...
void Teapots::resizeGL(int width, int height)
{
	...
	if (width <= height)
	{
		glOrtho(0.0, 20.0, 0.0, 20.0 * static_cast<GLfloat>(height) / static_cast<GLfloat>(width), -10.0, 10.0);
	}
	else
	{
		glOrtho(0.0, 20.0 * static_cast<GLfloat>(width) / static_cast<GLfloat>(height), 0.0, 20.0, -10.0, 10.0);
	}
...
void Teapots::paintGL()
{
	...
	GLfloat s = width() / static_cast<GLfloat>(fbObject->size().width());
	GLfloat t = height() / static_cast<GLfloat>(fbObject->size().height());
	...
	if (rubberBandIsShown)
	{
		...
		glRecti(rubberBandCorner1.x(), rubberBandCorner1.y(), rubberBandCorner2.x(), rubberBandCorner2.y());
