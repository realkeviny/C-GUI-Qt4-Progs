...
PumpSpreadsheet::PumpSpreadsheet(QWidget* parent) :QTableWidget(parent)
{
	clearData();
}
...
void PumpSpreadsheet::clearData()
{
	...
	setHorizontalHeaderItem(Company, new QTableWidgetItem(tr("Company")));
	setHorizontalHeaderItem(User, new QTableWidgetItem(tr("User")));
	setHorizontalHeaderItem(Quantity, new QTableWidgetItem(tr("Quantity")));
	setHorizontalHeaderItem(Status, new QTableWidgetItem(tr("Status")));
}

bool PumpSpreadsheet::addData(const QString& fileName, FileFormat format)
{
	if (format == Pump2000)
	{
	...
	{
		addDataInGasPumpXmlFormat(fileName);
	}
}

bool PumpSpreadsheet::saveData(const QString& fileName, FileFormat format)
{
	if (format == Pump2000)
	{
...
void PumpSpreadsheet::sortByColumn(int column, Qt::SortOrder order /* = Qt::AscendingOrder */)
{
...
void PumpSpreadsheet::computeTotals(Column column)
{
	...
	while (row<rowCount())
	{
		...
		{
			double newQuantity = text(row - 1, Quantity).toDouble() + text(row, Quantity).toDouble();
...
void PumpSpreadsheet::readTransactionElement(QXmlStreamReader& reader)
{
	...
	QStringList fields;
	fields << map.value("date") << map.value("time") << map.value("pump") << map.value("company") << map.value("user") << map.value("quantity") << map.value("status");
	addItem(fields);
}

bool PumpSpreadsheet::saveDataInGasPumpXmlFormat(const QString& fileName)
{
	QFile file(fileName);
	...
	if (file.error() != QFile::NoError)
	{
		showWarning(tr("Error when writing from %1:\n%2").arg(file.fileName()).arg(file.errorString()));
		return false;
	}
...
void PumpSpreadsheet::addItem(const QStringList& fields)
{
	...
	dateItem->setData(Qt::DisplayRole, QDate::fromString(fields[0], Qt::ISODate));
	setItem(row, Date, dateItem);	
	
	QTableWidgetItem* timeItem = new QTableWidgetItem;
	timeItem->setData(Qt::DisplayRole, QTime::fromString(fields[1], Qt::ISODate));
...
bool PumpSpreadsheet::matches(int row, const PumpFilter& filter) const
{
	...
	return (date >= filter.fromDate && date <= filter.toDate) && (time >= filter.fromTime && time <= filter.toTime) && (filter.pump.isEmpty() || pump == filter.pump) && (filter.company.isEmpty() || company == filter.company) && (filter.user.isEmpty() || user == filter.user) && (quantity >= filter.fromQuantity && quantity <= filter.toQuantity) && (filter.status.isEmpty() || status == filter.status);
}
...
PumpFilter::PumpFilter()
{
	...
	fromTime.setHMS(23, 59, 59);
	fromQuantity = 0.0;
	toQuantity = 10000000000000.0;
}