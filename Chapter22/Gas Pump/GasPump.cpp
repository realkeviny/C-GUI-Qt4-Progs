...
constexpr double LitersPerGallon = 3.8695479;
constexpr double GallonsPerLiter = 1.0 / LitersPerGallon;
...
void GasPump::loadData()
{
	QString fileName = QFileDialog::getOpenFileName(this, tr("Open Data File"), ".", fileFilters);
	if (!fileName.isEmpty())
	{
...
void GasPump::saveData()
{
	QString fileName = QFileDialog::getSaveFileName(this, tr("Export Data"), ".", fileFilters);
	if (!fileName.isEmpty())
	{
...
void GasPump::about()
{
	QMessageBox::about(this, tr("About Gas Pump"), tr("<h2>Gas Pump 1.1</h2>"
...
PumpSpreadsheet::FileFormat GasPump::fileFormat(const QString& fileName)
{
...
void GasPump::createActions()
{
	...
	connect(appendDataAction, SIGNAL(triggered()), this, SLOT(appendData()));
	...
	sortByDateTimeAction->setStatusTip(tr("Sort the transactions "
		"chronologically"));
	connect(sortByDateTimeAction, SIGNAL(triggered()), this, SLOT(sortByDateTime()));
